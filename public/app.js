...
async function fetchMfgMapPinPage(offset, limit) {
    ...
    if (!mfgMapPinStaticRows) {
        const response = await fetch('/data/mfg-companies-lite.json?v=20260425-1', { cache: 'no-store' });
        if (!response.ok) {
...
async function loadMfgEnrichmentOverrides() {
    try {
        const response = await fetch('/data/mfg-enrichment-overrides.json?v=20260428-1', { cache: 'no-store' });
        if (!response.ok) {
